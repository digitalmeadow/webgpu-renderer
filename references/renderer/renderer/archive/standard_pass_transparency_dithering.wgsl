...
fn fs_main(in: VertexOutput) -> FragmentOutput {
    ...
    for (var i = 0u; i < min(1u, max_lights_directional); i += 1u) {
        ...
        let shadow = fetch_shadow(cascade, shadow_coords);
    ...
    let x = i32(in.position.x) % bayerDimensions;
    let y = i32(in.position.y) % bayerDimensions;
...
fn random(vec: vec2<f32>) -> f32 {
    // Simple hash-based 2D random function
    let dot_val = dot(vec, vec2<f32>(12.9898, 78.233));
    return fract(sin(dot_val) * 43758.5453);
}

fn get_pcf_kernel() -> array<vec2<f32>, 4> {
    // 4-sample cross pattern; expand for more samples if needed
...
fn rotate_kernel(kernel: array<vec2<f32>, 4>, angle: f32) -> array<vec2<f32>, 4> {
    let c = cos(angle);
    let s = sin(angle);
    let rot = mat2x2<f32>(
        vec2<f32>(c, -s),
...
fn fetch_shadow(cascade_id: u32, homogeneous_coords: vec4<f32>) -> f32 {
    ...
    // PCF kernel and random rotation
    let kernel = get_pcf_kernel();
    let rand = random(light_local * 2048.0);
    let angle = rand * 6.2831853;
    let rotated_kernel = rotate_kernel(kernel, angle);
    let texel_size = shadow_map_texel_size();

    var shadow = 0.0;
    let blur_radius = 2.0;

    for (var i = 0u; i < 4u; i = i + 1u) {
        let offset = rotated_kernel[i] * texel_size * blur_radius;
        let sample_uv = light_local + offset;
        shadow = shadow + shadow_compare(sample_uv, cascade_id, depth);
    }
