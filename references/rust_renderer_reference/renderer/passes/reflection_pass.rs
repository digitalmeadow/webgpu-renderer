use wgpu::{
    ...
    RenderPipelineDescriptor, Sampler, SamplerBindingType, SamplerDescriptor, ShaderStages, StoreOp, SurfaceError, TextureDescriptor,
    TextureDimension, TextureSampleType, TextureUsages, TextureView, TextureViewDescriptor, TextureViewDimension, VertexState,
    include_wgsl,
};
...
pub struct ReflectionPass {
    reflection_pass_bind_group_layout: BindGroupLayout,
    reflection_pass_bind_group: Option<BindGroup>,
...
impl ReflectionPass {
    pub fn new(
        ...
        // Inputs
        let shader = surface_state.device.create_shader_module(include_wgsl!("reflection_pass.wgsl"));

        let reflection_pass_bind_group_layout = surface_state.device.create_bind_group_layout(&BindGroupLayoutDescriptor {
            label: Some("Reflection Pass Uniform Bind Group Layout"),
            entries: &[
                // Sampler Linear
        ...
        let ouput_color_texture = surface_state.device.create_texture(&TextureDescriptor {
            label: Some("Reflection Pass Output Color Texture"),
            size: Extent3d {
        ...
        let output_color_view = ouput_color_texture.create_view(&TextureViewDescriptor::default());

        return {
            Self {
                reflection_pass_bind_group_layout,
                reflection_pass_bind_group: None,
                render_pipeline,
                sampler_linear,
                output_color_view,
            }
    ...
    pub fn update_bind_groups(&mut self, surface_state: &SurfaceState, input_color_view: &TextureView) {
        self.reflection_pass_bind_group = Some(surface_state.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Reflection Pass Bind Group"),
            layout: &self.reflection_pass_bind_group_layout,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
    ...
    pub fn render(
        ...
        self.update_bind_groups(surface_state, &input_color_view);
...

        {
            let mut render_pass = encoder.begin_render_pass(&RenderPassDescriptor {
                label: Some("Reflection Pass Render Pass"),
            ...
            if let Some(bind_group) = &self.reflection_pass_bind_group {
