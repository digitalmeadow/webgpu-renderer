...
export class AABB {
  ...
  public updateWorldSpace(worldMatrix: Mat4): void {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const x = corner.data[0],
        y = corner.data[1],
        z = corner.data[2];

      const wx =
        worldMatrix.data[0] * x +
        worldMatrix.data[4] * y +
        worldMatrix.data[8] * z +
        worldMatrix.data[12];
      const wy =
        worldMatrix.data[1] * x +
        worldMatrix.data[5] * y +
        worldMatrix.data[9] * z +
        worldMatrix.data[13];
      const wz =
        worldMatrix.data[2] * x +
        worldMatrix.data[6] * y +
        worldMatrix.data[10] * z +
        worldMatrix.data[14];
